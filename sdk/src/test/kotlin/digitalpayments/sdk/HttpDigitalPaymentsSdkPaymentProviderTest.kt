...
class HttpDigitalPaymentsSdkPaymentProviderTest : AbstractSdkTest() {
    companion object {
    ...
    fun `given valid request when getPaymentProvider then return success ProviderList`() {
        ...
        StepVerifier
            .create(digitalPaymentsSdk.getPaymentProviders(SUPPLIER_ID, ACCESS_TOKEN))
            .expectNext(response)
            .verifyComplete()
    ...
    @Test
    fun `given FunctionalityNotAvailable when getPaymentProvider then throw BadRequest with statusCode is 400`() {
    ...
    @Test
    fun `given ClientTokenException when getPaymentProvider then throw BadRequest with statusCode is 400`() {
    ...
    @Test
    fun `given MissingFieldException when getPaymentProvider then throw BadRequest with statusCode is 400`() {
        val reason = ErrorReason.MISSING_FIELD_EXCEPTION
        val detailError = "Missing required field: XXX"
        val response = DigitalPaymentsDetailedError(reason, detailError)
    ...
    @Test
    fun `given TotalAmountReached when getPaymentProvider then throw BadRequest with statusCode is 400`() {
    ...
    @Test
    fun `given unexpected error when getPaymentProvider then throw InternalServerError with statusCode is 500`() {
